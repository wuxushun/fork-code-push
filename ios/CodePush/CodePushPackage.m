...
static NSString *ModulePath = @"main";
...
+ (void)clearUpdates
{
    [[NSFileManager defaultManager] removeItemAtPath:[self getModulePath] error:nil];
}
           ...
           failCallback:(void (^)(NSError *err))failCallback
{
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:newUpdateFolderPath]) {
        ...
        [[NSFileManager defaultManager] removeItemAtPath:newUpdateFolderPath
                                                   error:&error];
    } else if (![[NSFileManager defaultManager] fileExistsAtPath:[self getModulePath]]) {
        [[NSFileManager defaultManager] createDirectoryAtPath:[self getModulePath]
        ...
        NSURL *codePushURL = [NSURL fileURLWithPath:[self getModulePath]];
                                                        ...
                                                        if (isDiffUpdate) {
                                                            ...
                                                            for (NSString *patchedFileName in patchedFiles) {
                                                                ...
                                                                if ([[NSFileManager defaultManager] fileExistsAtPath:absoluteFilePath]
                                                                    ...
                                                                    NSString *patchedFilePath = [unzippedFolderPath stringByAppendingPathComponent:[NSString stringWithFormat:@"%@-patch", patchedFileName]];
                                                                    [[NSFileManager defaultManager] copyItemAtPath:absolutePatchedFilePath
                                                                                                            toPath:patchedFilePath
                                                                                                             error:&error];

                                                                    BOOL success = [HDiffPatch hdiffPatch:patchedFilePath origin:absoluteFilePath toDestination:absolutePatchedFilePath];
                                                                    [[NSFileManager defaultManager] removeItemAtPath:patchedFilePath
                                                                                                               error:&error];
                                                                    if (!success) {
...
{
    return [[self getModulePath] stringByAppendingPathComponent:DownloadFileName];
}
...
{
    return [[self getModulePath] stringByAppendingPathComponent:packageHash];
}
...
{
    return [[self getCodePushPath] stringByAppendingPathComponent:ModulePath];
}
...
{
    return [[self getModulePath] stringByAppendingPathComponent:StatusFile];
}

+ (NSString *)getUnzippedFolderPath
{
    return [[self getModulePath] stringByAppendingPathComponent:UnzippedFolderName];
}
