...
BUNDLE_FILE="$CONFIGURATION_BUILD_DIR/$BUNDLE_NAME"
...
if [[ $USE_HERMES != true ]]; then
  ...
  "$HERMES_CLI_PATH" -emit-binary $EXTRA_COMPILER_ARGS -out "$DEST/$BUNDLE_NAME" "$BUNDLE_FILE"
  if [[ $EMIT_SOURCEMAP == true ]]; then
