import { AcquisitionManager as Sdk } from "@iskytrip/code-push/script/acquisition-sdk";
...
async function checkForUpdate(deploymentKey = null, handleBinaryVersionMismatchCallback = null) {
  ...
  const diffVersion = await PackageMixins.getDiffVersion();
  const update = await sdk.queryUpdateWithCurrentPackage(queryPackage, diffVersion);
...
function getPromisifiedSdk(requestFetchAdapter, config) {
  ...
  sdk.queryUpdateWithCurrentPackage = (queryPackage, diffType) => {
    return new Promise((resolve, reject) => {
      module.exports.AcquisitionSdk.prototype.queryUpdateWithCurrentPackage.call(sdk, queryPackage, diffType, (err, update) => {
        if (err) {
          reject(err);
        } else {
          resolve(update);
